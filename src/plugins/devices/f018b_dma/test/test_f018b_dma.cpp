...
TEST_CASE(f018b_etrigmapd_address_translation) {
    // Verify $D706 (ETRIGMAPD) with MAP address translation
    F018bFixture f;

    // Create MapMmu for MAP address translation
    MapMmu mmu("mmu", &f.sparseBus);
    f.dma.setMapController(&mmu);

    // Setup MAP: blocks 0-2 enabled with offsets to physical memory
    MapState mapState;
    mapState.offsets[0] = 0x10000;  // Block 0 ($0000-$1FFF) -> physical $10000
    mapState.offsets[1] = 0x20000;  // Block 1 ($2000-$3FFF) -> physical $20000
    mapState.enables = 0x03;        // Enable blocks 0-1
    mapState.megabyteLow = 0;
    mapState.megabyteHigh = 0;
    mmu.setMapState(mapState);

    // Virtual DMA list at $2000 (will translate to physical $20000)
    uint32_t virt_list_addr = 0x2000;
    uint32_t phys_list_addr = 0x20000;

    // Setup source data at physical $10500
    f.sparseBus.write8(0x10500, 0xAA);
    f.sparseBus.write8(0x10501, 0xBB);

    // Destination at physical $30000 (no MAP translation)
    uint32_t phys_dst_addr = 0x30000;

    // DMA list format (inline with options):
    // Byte 0: options end marker
    // Byte 1-2: COPY command + count
    // Byte 3-5: source address
    // Byte 6-8: destination address
    // Byte 9-10: modulo

    f.sparseBus.write8(phys_list_addr + 0, 0x00);      // End of options
    f.sparseBus.write8(phys_list_addr + 1, 0x00);      // COPY command
    f.sparseBus.write8(phys_list_addr + 2, 0x02);      // Count = 2
    f.sparseBus.write8(phys_list_addr + 3, 0x00);

    // Source: virtual $0500 (translates to physical $10500)
    f.sparseBus.write8(phys_list_addr + 4, 0x00);      // Src LSB
    f.sparseBus.write8(phys_list_addr + 5, 0x05);      // Src MSB
    f.sparseBus.write8(phys_list_addr + 6, 0x00);      // Src BANK (no extra bits)

    // Destination: physical $30000 (direct, no translation)
    f.sparseBus.write8(phys_list_addr + 7, 0x00);      // Dst LSB ($00)
    f.sparseBus.write8(phys_list_addr + 8, 0x00);      // Dst MSB ($00)
    f.sparseBus.write8(phys_list_addr + 9, 0x03);      // Dst BANK ($03)

    f.sparseBus.write8(phys_list_addr + 10, 0x00);     // Modulo LSB
    f.sparseBus.write8(phys_list_addr + 11, 0x00);     // Modulo MSB

    // Trigger ETRIGMAPD with virtual address $2000
    // This should translate to physical $20000 and fetch the job from there
    ASSERT(f.dma.ioWrite(&f.flatBus, 0xD701, (virt_list_addr >> 8) & 0xFF));
    ASSERT(f.dma.ioWrite(&f.flatBus, 0xD702, 0x00));   // No bank bits needed for this test
    ASSERT(f.dma.ioWrite(&f.flatBus, 0xD706, virt_list_addr & 0xFF));  // ETRIGMAPD

    // Should trigger enhanced mode
    ASSERT(f.dma.isHaltRequested());

    // Execute DMA (multiple ticks to complete the transfer)
    for (int i = 0; i < 100; ++i) {
        f.dma.tick(1);
    }

    // Verify data was copied to destination
    uint8_t val0 = f.sparseBus.read8(phys_dst_addr + 0);
    uint8_t val1 = f.sparseBus.read8(phys_dst_addr + 1);
    ASSERT_EQ((int)val0, 0xAA);
    ASSERT_EQ((int)val1, 0xBB);
}
