    ...
    while (prefixCount < MAX_PREFIXES) {
        ...
        if (op == 0x42) {
            if (read8(m_state.pc) == 0x42) {
                // NEG/NEG detected. Behavior depends on experimental prefix mode:
                // Default (m_experimentalPrefixMode=false): Always consume as QUAD prefix (buggy behavior)
                // Experimental (m_experimentalPrefixMode=true): Only consume if next instruction supports QUAD

                bool shouldConsume = false;

                if (m_experimentalPrefixMode) {
                    // Experimental mode: Peek ahead to check if instruction supports QUAD
                    ...
                    // Default (buggy) mode: Always consume NEG/NEG as QUAD prefix
                    shouldConsume = true;
                }
                ...
                if (shouldConsume) {
                    ...
                    // NEG/NEG not consumed as prefix. Execute first NEG as regular instruction.
                    break;
                }
            }
            break;
